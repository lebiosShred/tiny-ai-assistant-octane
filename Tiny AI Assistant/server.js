...
const historyDir = process.env.HISTORY_DIR ? path.resolve(process.env.HISTORY_DIR) : path.join(__dirname, 'knowledge', 'history');
    ...
    function traverse(currentDir) {
        try {
            ...
            for (const item of items) {
                ...
                const relativePath = path.relative(knowledgeDir, fullPath).replace(/\\/g, '/');
                
                // Exclude history and history_test directories
                if (relativePath.startsWith('history') || relativePath.startsWith('history_test')) {
                    continue;
                }
                ...
                if (stat.isDirectory()) {
                    ...
                    allFiles.push({ name: relativePath, path: fullPath });
                }
            }
        } catch (e) {
            console.warn(`⚠️ Error traversing local KB dir ${currentDir}:`, e.message);
        }
    }
    
    traverse(knowledgeDir);
...
async function syncKnowledgeBaseFromGDrive() {
    ...
    try {
        ...
        let kbFolderId = process.env.KNOWLEDGE_BASE_FOLDER_ID;
        if (!kbFolderId) {
        ...
        for (const dbDoc of dbDocs) {
            if (!remoteFileIds.has(dbDoc.fileId)) {
                ...
                // Also purge from local knowledge base directory if it exists
                if (dbDoc.prospectName) {
                    const localPath = path.join(__dirname, 'knowledge', dbDoc.prospectName);
                    try {
                        ...
                        console.warn(`⚠️ Failed to delete local file ${localPath}:`, fsErr.message);
                    }
                }
            }
        }
        ...
        for (const file of remoteFiles) {
            try {
                ...
                const localPath = file.path ? path.join(__dirname, 'knowledge', file.path) : null;
                const existsLocally = localPath && fs.existsSync(localPath);
                ...
                if (cachedDoc && cachedDoc.contentHash === fileHash) {
                    ...
                    // If missing locally, restore from DB cache content
                    if (localPath && !existsLocally && cachedDoc.extractedProfile) {
                        try {
                            const localDirPath = path.dirname(localPath);
                            if (!fs.existsSync(localDirPath)) {
                                fs.mkdirSync(localDirPath, { recursive: true });
                            }
                            fs.writeFileSync(localPath, cachedDoc.extractedProfile, 'utf8');
                            console.log(`💾 Restored missing local file from DB cache: ${localPath}`);
                        } catch (fsErr) {
                            console.warn(`⚠️ Failed to restore local file ${localPath}:`, fsErr.message);
                        }
                    }
                    continue;
                }
                ...
                if (content && content.trim()) {
                    ...
                    // Write to local knowledge base directory
                    if (localPath) {
                        try {
                            const localDirPath = path.dirname(localPath);
                            if (!fs.existsSync(localDirPath)) {
                            ...
                            console.log(`💾 Successfully synchronized local file: ${localPath}`);
                        } catch (fsErr) {
                            console.warn(`⚠️ Failed to write local file ${localPath}:`, fsErr.message);
                        }
                    }
                }
            } catch (fileErr) {
...
function logAuditEvent(req, action, details = {}) {
    ...
    const logFilePath = path.join(__dirname, 'knowledge', 'audit_log.jsonl');
...
`;
                        ...
                        execute: async ({ company, prospect_name }) => {
                            ...
                            if (gdriveAvailable) {
                                try {
                                    ...
                                    const rootFolderId = process.env.GDRIVE_ROOT_FOLDER_ID || 'root';
                                    let prospectsFolderId = process.env.COMPANY_FOLDER_ID;
                                        ...
                                        for (const folder of matchedFolders) {
                                            if (cleanProspect) {
                                                ...
                                                if (subFiles.length > 0) {
                                                    // Invalidate content caches of any files inside this folder
                                                    try {
                                                        const innerFiles = await gdriveService.listFolderRecursive(subFiles[0].id);
                                                        for (const f of innerFiles) {
                                                            gdriveService.invalidateFileContentCache(f.id);
                                                        }
                                                    } catch (cacheErr) {
                                                        console.warn('⚠️ File content cache invalidation failed:', cacheErr.message);
                                                    }
                                                ...
                                                // Invalidate content caches of any files inside this folder
                                                try {
                                                    const innerFiles = await gdriveService.listFolderRecursive(folder.id);
                                                    for (const f of innerFiles) {
                                                        gdriveService.invalidateFileContentCache(f.id);
                                                    }
                                                } catch (cacheErr) {
                            ...
                            // Database document companion metadata cache purge
                            try {
                                const { eq, and } = require('drizzle-orm');
                                if (cleanProspect) {
                                    await db.delete(documentCompanionMetadata)
                                        .where(
                                            and(
                                                eq(documentCompanionMetadata.company, company),
                                                eq(documentCompanionMetadata.prospectName, prospect_name)
                                            )
                                        );
                                } else {
                                    await db.delete(documentCompanionMetadata)
                                        .where(eq(documentCompanionMetadata.company, company));
                                }
                                console.log(`🗑️ Purged database companion metadata cache for company "${company}"`);
                            } catch (metaDbErr) {
                                console.warn(`⚠️ Postgres companion metadata purge failed during folder deletion:`, metaDbErr.message);
                            }
    ...
    if (pathname === '/api/questions') {
        const questionsFile = path.join(__dirname, 'knowledge', 'custom-questions.json');
                ...
                try {
                    ...
                    if (!folderId && !company) {
                        ...
                        if (envProspectsId) {
                            ...
                            // Resolve the Prospects or Company folder under root, and list folders inside it
                            const rootFolderId = process.env.GDRIVE_ROOT_FOLDER_ID || 'root';
                            const prospectsSearch = await drive.files.list({
                                q: `(name = 'Company' or name = 'Prospects') and mimeType = 'application/vnd.google-apps.folder' and '${rootFolderId}' in parents and trashed = false`,
                                fields: 'files(id, name)',
                            ...
                            if (prospectsFiles.length === 0) {
                                console.log(`⚠️ Central folder not found under parents '${rootFolderId}'. Searching globally...`);
                                const fallbackSearch = await drive.files.list({
                                    q: `(name = 'Company' or name = 'Prospects') and mimeType = 'application/vnd.google-apps.folder' and trashed = false`,
                                    fields: 'files(id, name)',
                                    pageSize: 1
                                });
...
If data for a field is missing or cannot be inferred, inject "[UNKNOWN]".`;
    ...
    if (pathname === '/api/knowledge/restore' && req.method === 'POST') {
        const knowledgeDir = path.join(__dirname, 'knowledge');
        const backupDir = path.join(__dirname, 'knowledge_backup');
...
${payload.intakeAnswers || ''}`;
    ...
    if (pathname.startsWith('/v2')) {
        ...
        const distDir = path.join(__dirname, 'demo-v2', 'dist');
    ...
    if (pathname.startsWith('/book')) {
        ...
        if (pathname === '/' || pathname === '') {
            ...
            relativePath = path.join('/Tiny AI Assistant', pathname);
        }
    }
